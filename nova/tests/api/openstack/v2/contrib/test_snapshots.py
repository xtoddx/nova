...
def stub_snapshot_delete(self, context, snapshot):
    global _last_param
    _last_param = dict(snapshot_id=snapshot['id'])
...
def stub_volume_get(self, context, volume_id):
    LOG.debug(_("_get_volume: %s"), locals())
    if str(volume_id) != '12':
        raise exception.NotFound
    param = _get_default_volume_param()
    param['id'] = volume_id
    return param
...
class SnapshotApiTest(test.TestCase):
    def setUp(self):
        ...
        route = v2_app.map.match('/1234/os-snapshots')
        controller = route['controller'].controller
        controller_api = controller.volume_api
        self.stubs.SmartSet(controller_api, "create_snapshot", stub_snapshot_create)
        self.stubs.SmartSet(controller_api, "create_snapshot_force",
            stub_snapshot_create)
        self.stubs.SmartSet(controller_api, "delete_snapshot", stub_snapshot_delete)
        self.stubs.SmartSet(controller_api, "get_snapshot", stub_snapshot_get)
        self.stubs.SmartSet(controller_api, "get", stub_volume_get)
        self.stubs.SmartSet(controller_api, "get_all_snapshots",
    ...
    def test_snapshot_create(self):
        ...
        LOG.debug(_("test_snapshot_create: CALLING WITH %s"), snapshot)
